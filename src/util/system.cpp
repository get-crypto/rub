...
const char * const BITCOIN_CONF_FILENAME = "rub.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //rub-tx using stdin
        ...
        if (key[0] != '-') {
            if (!m_accept_any_command && m_command.empty()) {
                // The first non-rub arg is a registered command
...
{
    // Windows: C:\Users\Username\AppData\Roaming\RubCore
    // macOS: ~/Library/Application Support/RubCore
    // Unix-like: ~/.rubcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RubCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/RubCore";
#else
    // Unix-like
    return pathRet / ".rubcore";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty rub.conf if it does not exist
