...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Rub address"));
    ...
    coinJoinCoinsAction->setStatusTip(tr("Send %1 funds to a Rub address").arg(strCoinJoinName));
    coinJoinCoinsAction->setToolTip(coinJoinCoinsAction->statusTip());

    receiveCoinsAction = new QAction(tr("&Receive"), this);
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and rub: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Rub addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Rub addresses"));
    ...
    openAction->setStatusTip(tr("Open a rub: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Rub command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (fNetworkActive) {
        ...
        tooltip = tr("%n active connection(s) to Rub network", "", count);
    } else {
...
void BitcoinGUI::updateWidth()
{
    ...
    // Use nButtonsVisible + 1 <- for the rub logo
