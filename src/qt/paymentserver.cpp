...
const QString BITCOIN_IPC_PREFIX("rub:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // rub: URI
        {
    ...
    optionsModel(nullptr)
{
    ...
    // on Mac: sent when you click rub: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start rub: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling rub: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("rub://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'rub://' is not a valid URI. Use 'rub:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // rub: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Rub address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
