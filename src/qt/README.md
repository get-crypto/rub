This directory contains the source code for the Rub Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Rub UI.
...
- ClientModel is used by the main application `rubgui` and several models like `peertablemodel`.
...
#### paymentserver.(h/cpp)
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a rub:... link from a browser).

Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a rub:... link from a browser).
...
**Note:** Do not change `local/rub_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `rub-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `rub-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `rub-qt`
9. You're all set! Start developing, building, and debugging the Rub Core GUI
