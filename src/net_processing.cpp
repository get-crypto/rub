 ...
 *  We have 4 times smaller block times in Rub, so we need to push 4 times more invs per 1MB. */
...
struct Peer {
     ...
     * (Rub)    Enables the flag that allows GetTxRelay() to return m_tx_relay */
...
private:
     ...
     * (Rub)    Always initialized but selectively available through GetTxRelay()
...
bool PeerManagerImpl::AlreadyHave(const CInv& inv)
{
    ...
    /*
        Rub Related Inventory Messages
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        // RUB this code also handles non-TXs (Rub specific messages)
