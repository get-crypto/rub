...
impl PrivateKey {
    ...
    pub fn generate_rub(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "rub_helpers")]
    pub fn generate_rub_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_rub(rng))
            .collect()
    }
