...
- [Development guidelines](#development-guidelines)
    - [General Rub Core](#general-rub-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Rub Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while rubd is running with the `logging`
RPC.  For instance, launching rubd with `-debug` or `-debug=1` will turn on
...
Rub Core is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run rubd.

You can then use the `-debug=lock` configuration option at rubd startup or
`rub-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `rub-cli logging [] '["lock"]'`.
...
([`valgrind.supp`](https://github.com/rubpay/rub/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_rub
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_rub --log_level=test_suite
$ valgrind -v --leak-check=full src/rubd -printtoconsole
...
# A coverage report will now be accessible at `./test_rub.coverage/index.html`.
```
...
To profile a running rubd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep rubd` -- sleep 60
```
...
Rub Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`rubd`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
However, in open source software such as Rub Core, where everyone uses
...
pay attention to for reviewers of Rub Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Rub Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Rub Core, in which case
...
a Rub Core subtree, so that they can be integrated quickly. Cosmetic changes
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `rub-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `rub-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Rub Core, the wallet was always
...
- Use *invalid* Rub addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
