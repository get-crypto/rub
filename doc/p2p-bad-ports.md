When Rub Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no Rub node is listening,
or one where a service is listening that is not related to the Rub network.
As a result, this service may occasionally get connection attempts from Rub
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by Rub Core,
trying to connect because it thinks there is a Rub node on that
...
They are also considered "bad" ports as they require clients to either run Rub
...
Below is a list of "bad" ports which Rub Core avoids when choosing a peer to
