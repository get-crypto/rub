...
Below are some notes on how to build Rub Core for Windows.

The options known to work for building Rub Core on Windows are:
...
Note that for WSL the Rub Core source path MUST be somewhere in the default mount file system, for
example /usr/src/rub, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\rub`, for example:

    make install DESTDIR=/mnt/c/workspace/rub
