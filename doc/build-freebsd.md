...
This guide describes how to build rubd, command-line utilities, and GUI on FreeBSD.
...
### 2. Clone Rub Repo
Now that `git` and all the required dependencies are installed, let's clone the Rub Core repository to a directory. All build scripts and commands will run from this directory.
``` bash
...
```
to: /path/to/rub/depends/x86_64-unknown-freebsd[release-number]
```
...
Rub Core includes a GUI built with the cross-platform Qt Framework. To compile the GUI, we need to install `qt5`. Skip if you don't intend to use the GUI.
```bash
pkg install qt5
```
###### libqrencode
...
Rub Core can provide notifications via ZeroMQ. If the package is installed, support will be compiled in.
```bash
pkg install libzmq4
```
