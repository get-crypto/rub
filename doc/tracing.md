# User-space, Statically Defined Tracing (USDT) for Rub Core

Rub Core includes statically defined tracepoints to allow for more
...
**Note**: Rub Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Rub Core
...
Multiple tools can list the available tracepoints in a `rubd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Rub Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/rubd
stap net        outbound_message 0x0000000000107c05 /src/rubd
stap validation block_connected  0x00000000002fb10c /src/rubd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Rub Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/rubd | grep NT_STAPSDT -A 4 -B 2
