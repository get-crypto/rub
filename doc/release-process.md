...
* [ ] Update translations, see [translation_process.md](https://github.com/rubpay/rub/blob/develop/doc/translation_process.md#synchronising-translations).
* [ ] Update manpages (after rebuilding the binaries), see [gen-manpages.py](https://github.com/rubpay/rub/blob/develop/contrib/devtools/README.md#gen-manpagespy).
* [ ] Update rub.conf and commit, see [gen-rub-conf.sh](https://github.com/rubpay/rub/blob/develop/contrib/devtools/README.md#gen-rub-confsh).

Before every minor and major release:

* [ ] Review ["Needs backport" labels](https://github.com/rubpay/rub/labels?q=backport).
* [ ] Update DIPs with any changes introduced by this release (see [this pull request](https://github.com/rubpay/dips/pull/142) for an example)
...
* [ ] Update hardcoded [seeds](/contrib/seeds/README.md), see [this pull request](https://github.com/rubpay/rub/pull/5914) for an example.
* [ ] Update [`src/chainparams.cpp`](/src/chainparams.cpp) `m_assumed_blockchain_size` and `m_assumed_chain_state_size` with the current size plus some overhead (see [this](#how-to-calculate-assumed-blockchain-and-chain-state-size) for information on how to calculate them).
* [ ] Update [`src/chainparams.cpp`](/src/chainparams.cpp) `chainTxData` with statistics about the transaction count and rate. Use the output of the `getchaintxstats` RPC, see
  [this pull request](https://github.com/rubpay/rub/pull/5692) for an example. Reviewers can verify the results by running `getchaintxstats <window_block_count> <window_last_block_hash>` with the `window_block_count` and `window_last_block_hash` from your output.
...
git clone https://github.com/rubpay/guix.sigs.git
git clone https://github.com/rubpay/rub-detached-sigs.git
git clone https://github.com/rubpay/rub.git
```

### Rub Core maintainers/release engineers, suggestion for writing release notes
...
Checkout the Rub Core version you'd like to build:
...
_Note: this step can be skipped if [our CI](https://github.com/rubpay/rub/blob/master/ci/test/00_setup_env.sh#L64) still uses bitcoin's SDK package (see SDK_URL)_
...
* Transfer `rubcore-osx-unsigned.tar.gz` to macOS for signing
* Extract and sign:

    ```sh
    tar xf rubcore-osx-unsigned.tar.gz
...
* Extract and sign:
    ...
    ```sh
    tar xf rubcore-win-unsigned.tar.gz
...
However if this is done, once the release has been tagged in the rub-detached-sigs repo, the `guix-codesign` step must be performed again in order for the guix attestation to be valid when compared against the attestations of non-codesigner builds.

### Windows and macOS codesigners only: Commit the detached codesign payloads

```sh
pushd ~/rub-detached-sigs
...
- Detached signatures will then be committed to the [rub-detached-sigs](https://github.com/rubpay/rub-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
* [ ] Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to GitHub as GitHub draft release.
    1. The contents of each `./rub/guix-build-${VERSION}/output/${HOST}/` directory, except for
       `*-debug*` files.
       ...
       as save storage space *do not upload these to the rub.org server*.

       ```sh
       find guix-build-${VERSION}/output/ -maxdepth 2 -type f -not -name "SHA256SUMS.part" -and -not -name "*debug*" -exec scp {} user@rub.org:/var/www/bin/rub-core-${VERSION} \;
       ```
...
* [ ] Update the rub.org download links
* [ ] Ensure that docker hub images are up to date

### Announce the release:
* [ ] Release on Rub forum: https://www.rub.org/forum/topic/official-announcements.54/ (necessary so we have a permalink to use on twitter, reddit, etc.)
...
### After the release:
* [ ] Submit patches to BTCPay to ensure they use latest / compatible version see https://github.com/rubpay/rub/issues/4211#issuecomment-966608207
* [ ] Update Core and User docs (docs.rub.org)
* [ ] Test Docker build runs without error in Rubmate
...
```sh
xcrun notarytool submit rubcore-{version}-{x86_64, arm64}-apple-darwin.dmg --keychain-profile "AC_PASSWORD" --wait
```
...
After successfully notarizing the .dmg file, extract `Rub-Qt.app` from the .dmg.
To verify that the notarization process was successful, run the following command:

```sh
spctl -a -vv -t install Rub-Qt.app
```

Replace `Rub-Qt.app` with the path to your .app file. This command checks whether your .app file passes Gatekeeper’s
