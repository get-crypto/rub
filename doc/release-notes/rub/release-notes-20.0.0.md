...
Currently, 10% of block rewards are set aside for the Rub DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking Rub in a
credit pool. This credit pool mirrors the available credit supply in Rub
Platform. When a certain amount of Rub is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on Rub
Platform. Masternode identities receive the fees and can convert them back to
Rub by withdrawing them from Rub Platform.
...
### Removal of reject network messages from Rub Core (BIP61)

This feature has been disabled by default since Rub Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since Rub Core version 20.0.0 there are extra changes:
...
- It is possible to run Rub Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `RUB/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from Rub Core (BIP61)_](#removal-of-reject-network-messages-from-rub-core-bip61) above for
...
### Rub-specific changes in existing RPCs:
...
- The `rub-cli -getinfo` command now displays the wallet name and balance for
...
Please check `Help -> Command-line options` in Qt wallet or `rubd --help` for
more information.
...
python test framework and `rub-cli` tool both take advantage of this, so
for example:

```sh
rub-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
rub-cli -named createwallet mywallet load_on_startup=1
```
...
with Rub’s product needs, such as SegWit and RBF, are excluded from our
...
[DIP-0023]: https://github.com/rubpay/dips/blob/master/dip-0023.md
[DIP-0029]: https://github.com/rubpay/dips/blob/master/dip-0029.md

[i2p]: https://github.com/rubpay/rub/blob/v20.x/doc/i2p.md
[guix-readme]: https://github.com/rubpay/rub/blob/v20.x/contrib/guix/README.md
[json-rpc]: https://github.com/rubpay/rub/blob/v20.x/doc/JSON-RPC-interface.md#parameter-passing
[set-of-changes]: https://github.com/rubpay/rub/compare/v19.3.0...rubpay:v20.0.0
