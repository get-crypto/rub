...
Rub Core imperfectly follows semantic versioning. Breaking changes should be
...
Support for the BIP70 Payment Protocol has been dropped from Rub Qt.
...
The systemd init file (`contrib/init/rubd.service`) has been changed to use
`/var/lib/rubd` as the data directory instead of `~rub/.rub`. This
change makes Rub Core more consistent with other services, and makes the
...
`datadir` option in `/etc/rub/rub.conf` will have no effect. This is
because the command line arguments specified in the init files take precedence
over the options specified in `/etc/rub/rub.conf`.
...
See the [release notes from v18.0.1](https://github.com/rubpay/rub/blob/v18.0.1/doc/release-notes.md) for a full description of the changes from the
...
the deprecation of wallet accounts in RubCore v0.17 and introduction of PSBT
format.
...
- The `testnet` field in `rub-cli -getinfo` has been renamed to `chain` and now returns the current network name as defined in BIP70 (main, test, regtest).
...
This release introduces many hundreds updates from Bitcoin v0.18/v0.19/v0.20/v0.21/v22. Bitcoin changes that do not align with Rub’s product needs, such as SegWit and RBF, are excluded from our backporting. For additional detail on what’s included in Bitcoin, please refer to their release notes.

v18.1.0 Change log
==================

See detailed [set of changes](https://github.com/rubpay/rub/compare/v18.0.2...rubpay:v18.1.0).
