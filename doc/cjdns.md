# CJDNS support in Rub Core

It is possible to run Rub Core over CJDNS, an encrypted IPv6 network that
...
done outside of Rub Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Rub network and individual nodes.
...
before it will work with your Rub Core node. This is described in steps
...
## Run Rub Core with CJDNS

Once you are connected to the CJDNS network, the following Rub Core
...
When enabled, this option tells Rub Core that it is running in an
...
IPv6 local network. This helps Rub Core perform better address management:
...
CJDNS support was added to Rub Core in version 21.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `rub-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/rubpay/rub/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Rub Core

There are several ways to see your CJDNS address in Rub Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `rub-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `rub-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `rub-cli help getnodeaddresses` for details.
