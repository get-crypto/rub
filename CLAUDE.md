# Rub Core Development Guide

## Overview

Rub Core is the reference implementation for Rub, a cryptocurrency. It builds on top of Bitcoin Core, a codebase that
is primarily written in C++20 (requiring at least Clang 16 or GCC 11.1). Rub Core uses the GNU Autotools build system.
...
- **Implementation** `src/` - C++20 codebase
  ...
  - `src/qt/` - Implementation of Rub Qt, the GUI (uses Qt 5)
...
- **Functional Tests**: `test/functional/` - Python tests (minimum version in `.python-version`) dependent on `rubd` and `rub-node`
...
- Vendored dependencies:
  - `src/{crc32c,rubbls,gsl,immer,leveldb,minisketch,secp256k1,univalue}`
...
./src/test/test_rub --run_test=getarg_tests

# Debug unit tests
gdb ./src/test/test_rub
```
...
Rub Core extends Bitcoin Core through composition, using a layered architecture:
...
#### Rub-Specific Databases

- **CFlatDB**: A Rub-specific flat file database format used for persistent storage
...
- **CDBWrapper**: Bitcoin Core database wrapper extended for Rub-specific data
...
2. **Parameter Interaction**: Rub-specific configuration validation
3. **Interface Setup**: Rub manager instantiation in NodeContext
...
- **ChainstateManager**: Enhanced with Rub-specific validation
...
# Run rubd with debug logging
./src/rubd -debug=all -printtoconsole

# Run functional test with custom rubd
test/functional/test_runner.py --rubd=/path/to/rubd
...
# Memory debugging
valgrind --leak-check=full ./src/rubd
```
...
gh api repos/rubpay/rub/actions/jobs/<JOB_ID>/logs
...
# gh api repos/rubpay/rub/actions/jobs/46274126203/logs
```
...
- Rub uses `unordered_lru_cache` for efficient caching with LRU eviction
- The codebase extensively uses Rub-specific data structures for performance
