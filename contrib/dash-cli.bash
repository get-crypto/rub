# bash programmable completion for rub-cli(1)
# Copyright (c) 2012-2019 The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $rub-cli for RPC
_rub_rpc() {
...
_rub_cli() {
    ...
    # save and use original argument to invoke rub-cli for -help, help and RPC
    # as rub-cli might not be in $PATH
    rub_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($rub_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_rub_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
