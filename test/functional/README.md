...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x500=12500 RUB) in its wallet. Using them is much more
...
with the rubd(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a rubd.  `P2PInterface`
...
Test objects for interacting with a rubd node over the p2p interface.
