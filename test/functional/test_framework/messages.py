...
BIP125_SEQUENCE_NUMBER = 0xfffffffd  # Sequence number that is BIP 125 opt-in and BIP 68-opt-out -- not used in RUB
...
# Objects that map to rubd objects, which can be serialized/deserialized
...
class CBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(rubhash(r))
            self.hash = rubhash(r)[::-1].hex()
...
class CompressibleBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(rubhash(r))
            self.hash = int(rubhash(r)[::-1].hex(), 16)
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in rubd indicates these should be deserialized as blocks
