This directory contains integration tests that test rubd and its
...
rubd and rub-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (rub-tx, ...).
...
Before tests can be run locally, Rub Core must be built.  See the [building instructions](/doc#building) for help.
...
Many Rub specific tests require rub_hash. To install it:

- Clone the repo `git clone https://github.com/rubpay/rub_hash`
- Install rub_hash `cd rub_hash && pip3 install -r requirements.txt .`
...
The P2P and RPC ports used by the rubd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another rubd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its rubd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other rubd processes are running.

On linux, the test framework will warn if there is another
rubd process running when the tests are started.

If there are zombie rubd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
rubd processes running on the system, so should not be used if any non-test
rubd processes are being run.**
...
sure rubd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and rubd `debug.log`s will all be dumped
...
`test_framework.log` and rubd `debug.log`s can be combined into a single
...
call methods that interact with the rubd nodes-under-test.

If further introspection of the rubd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/rubd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/rubd <pid>
```
